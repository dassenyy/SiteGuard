...
namespace SiteGuard.Commands {
    public class OfflineSeeRecordsCommand : ICommand, IUsageProvider {
        public string Command => "oseerecords";
        public string[] Aliases { get; } = { "ocrecords" };
        public string Description => Plugin.Instance.Translation.OfflineSeeRecordsCommandDescription;
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response) {
            ...
            if (entryTypes.Contains("report")) {
                entries = entries.Concat(Plugin.Database.GetReportsByTargetId(targetId)).ToList();
            } if (entryTypes.Contains("warn")) {
                entries = entries.Concat(Plugin.Database.GetWarnsByTargetId(targetId)).ToList();
            } if (entryTypes.Contains("kick")) {
                entries = entries.Concat(Plugin.Database.GetKicksByTargetId(targetId)).ToList();
            } if (entryTypes.Contains("ban")) {
                entries = entries.Concat(Plugin.Database.GetIdBansByTargetId(targetId)).ToList();
            }
